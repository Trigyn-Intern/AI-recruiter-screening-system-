...
assert any(
    r["file"] in {"api_py", "api.py", "append_api_py", "backend_py"}
    for r in status["files"]
), status["files"]
...
logs = parse_log_signals(
    (REPO / "reports" / "ci-logs.txt").read_text(encoding="utf-8", errors="replace")
)
print(
    "Log signals :",
    logs["python_unit_tests"],
    logs["python_integration_tests"],
    logs["coverage"],
)
