...
def get_git_info():
    """Read the GitHub repo path and current branch from ``git``, env, or metadata.json."""
    repo_path = os.environ.get("GITHUB_REPOSITORY")
    branch = os.environ.get("GITHUB_REF_NAME") or os.environ.get("GITHUB_BRANCH")
    ...
    if not repo_path and remote_url:
    ...
    if not repo_path:
        ...
        if meta_path.exists():
            try:
                ...
                if "Imported from GitHub:" in desc:
                    repo_path = desc.split("Imported from GitHub:", 1)[1].strip()
            except Exception:
                pass

    if not repo_path:
        repo_path = "Trigyn-Intern/AI-recruiter-screening-system-"
