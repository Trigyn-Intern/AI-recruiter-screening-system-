...
function Login() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (role === "manager" || role === "admin" || trimmedEmail === "manager@local.test") {
        localStorage.setItem("testing.token", data.token);
        localStorage.setItem("testing.user", JSON.stringify(data.user || {}));

        const testingUrl = import.meta.env.VITE_TESTING_APP_URL;
        if (testingUrl && !testingUrl.includes("5174")) {
