...
def render_structured(data):
    ...
    for key, label in [
        ...
        out.append(
            ...
            + html.escape(label)
            + "</dt><dd>"
            + html.escape(str(data.get(key, "") or "-"))
            + "</dd>"
        )
    out.append("</dl>")
    ...
    if rows:
        ...
        for r in rows:
            ...
            out.append(
                "<tr><td>"
                + html.escape(str(r.get("checkItem", "")))
                + '</td><td><span class="pill '
                + pill
                + '">'
                + html.escape(str(r.get("status", "")))
                + "</span></td><td>"
                + html.escape(str(r.get("notes", "")))
                + "</td></tr>"
            )
    ...
    if has_sec and sec_rows:
        out.append(
            "<table><thead><tr><th>Vulnerability Check</th><th>Status</th><th>Comments</th></tr></thead><tbody>"
        )
        for r in sec_rows:
            pill = pill_for(r.get("status", ""))
            out.append(
                ...
                + html.escape(str(r.get("status", "")))
                + "</span></td><td>"
                + html.escape(str(r.get("comments", "")))
                + "</td></tr>"
            )
        out.append("</tbody></table>")
    else:
        out.append(
            '<p class="note">No security-specific checks were included in this review.</p>'
        )
    ...
    if perf:
        ...
        for p in perf:
            out.append(
                "<li><strong>"
                + html.escape(str(p.get("title", "")))
                + "</strong>: "
                + html.escape(str(p.get("details", "")))
                + "</li>"
            )
        out.append("</ul>")
    ...
    if style:
        ...
        for s in style:
            out.append(
                "<li><strong>"
                + html.escape(str(s.get("practice", "")))
                + "</strong>: "
                + html.escape(str(s.get("issuesFound", "")))
                + "</li>"
            )
        out.append("</ul>")
    out.append('<h2>Test Coverage</h2><dl class="grid">')
    out.append(
        "<dt>Are tests present?</dt><dd>"
        + html.escape(str(data.get("hasTests", "-")))
        + "</dd>"
    )
    out.append(
        "<dt>Test Coverage % (approx)</dt><dd>"
        + html.escape(str(data.get("coveragePercent", "-")))
        + "</dd>"
    )
    out.append(
        "<dt>Manual Tests Needed</dt><dd>"
        + html.escape(str(data.get("manualTestNotes", "-")))
        + "</dd></dl>"
    )
    ...
    if fb:
        ...
        for f in fb:
            out.append(
                "<li><strong>"
                + html.escape(str(f.get("reviewerName", "")))
                + "</strong>: "
                + html.escape(str(f.get("comment", "")))
                + "</li>"
            )
        out.append("</ul>")
    if data.get("finalNotes"):
        out.append(
            "<h2>Final Notes</h2><p>" + html.escape(str(data["finalNotes"])) + "</p>"
        )
    out.append('<h2>Approval</h2><dl class="grid">')
    out.append(
        "<dt>Approved by</dt><dd>"
        + html.escape(str(data.get("approvedBy", "-")))
        + "</dd>"
    )
    out.append(
        "<dt>Date of Approval</dt><dd>"
        + html.escape(str(data.get("approvalDate", "-")))
        + "</dd>"
    )
    merge = data.get("mergeStatus", "-") or "-"
    pill = pill_for(merge)
    out.append(
        '<dt>Merge Status</dt><dd><span class="pill '
        + pill
        + '">'
        + html.escape(str(merge))
        + "</span></dd></dl>"
    )
    return "\n".join(out)
...
def render_detailed(data=None):
    ...
    out.append(
        '<p class="meta">Tick each box. Leave a one-line note for anything unchecked. Link to a follow-up issue if the fix is out of scope for this PR.</p>'
    )
    import re as _re
    ...
    for name, items in SECTIONS:
        out.append(
            '<div class="checklist-section"><h2>' + html.escape(name) + "</h2><ul>"
        )
        for it in items:
            n = _norm(it)
            p_ = it[:40].lower()
            ...
            box = (
                ...
                if checked
                else '<input type="checkbox" s/>'
            )
            note = ' <span class="llm-tick">verified by LLM</span>' if checked else ""
            out.append(
                '<li class="checklist-item"><label class="checkbox-row">'
                + box
                + html.escape(it)
                + note
                + "</label></li>"
    ...
    out.append(
        '<div class="red-flag">Unchecked array access without bounds validation</div>'
    )
    out.append('<div class="red-flag">Missing <code>await</code> on async calls</div>')
    out.append("<h3>Performance: N+1 queries</h3>")
    out.append(
        '<div class="heuristic">Performance issues in code review are often subtle. The most common culprit: a loop that queries the database on every iteration.</div>'
    )
    out.append("<pre><code>" + html.escape(N1) + "</code></pre>")
    out.append("<h3>Testing heuristic</h3>")
    out.append(
        '<div class="heuristic">If the PR description says "fix X bug," there should be a test that would have caught the bug before the fix.</div>'
    )
    out.append("<h3>Tooling notes</h3><ul>")
    out.append(
        "<li><strong>DevPlaybook AI Code Review</strong> - paste your diff and get automated security analysis before human review.</li>"
    )
    out.append(
        "<li><strong>DevPlaybook Code Diff</strong> - compare before/after versions of changed files when reviewing locally.</li>"
    )
    out.append("</ul>")
    return "\n".join(out)
...
def main():
    ...
    doc = "\n".join(
        [
            "<!doctype html>",
            '<html lang="en">',
            '<head><meta charset="utf-8"><title>Code Review Checklist - '
            + html.escape(stamp)
            + "</title><style>"
            + CSS
            + "</style></head>",
            "<body>",
            '<header><h1>Code Review Checklist</h1><p class="meta">Generated '
            + html.escape(stamp)
            + " by the <code>code-review-policy</code> skill.</p></header>",
            structured_html,
            detailed_html,
